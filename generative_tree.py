...
class GenerativeTree():
    ...
    def __init__(self, input: list, scope_structure: ScopeStructure, functions, global_variables):
        depth = 0
        previous_node = None
        ...
        for node_str_raw in input:
            ...
            if (num_of_leading_spaces == 0):
                self.root_node = Node(node_str, parent_node=None, scope_structure=self.scope_structure, 
                                        root_node=True, functions=self.functions, global_variables=self.global_variables)
            ...
            # siblings
                ...
                new_node = Node(node_str, parent_node=previous_node.parent_node, scope_structure=self.scope_structure,
                                functions=self.functions, global_variables=self.global_variables)
